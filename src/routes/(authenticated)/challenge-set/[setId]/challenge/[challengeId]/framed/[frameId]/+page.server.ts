...
export const load: PageServerLoad = async ({ parent, params }) => {
	...
	const frameId = parseInt(params.frameId);
	const orderedChildren = CHLG.orderedChildren(challenge);
	const frameChallenge = challenge.children.find((c) => c.id === frameId);
	const challengeHasAnotherFrame = orderedChildren.at(-1)?.id !== frameId;
	const responses = orderedChildren.map((child) => {
		const givenResponse = CHLG.response(child);
		return {
			response: givenResponse,
			isCorrect: child.acceptedResponsesIfOpen.includes(givenResponse),
		};
	});

	if (!frameChallenge) {
		throw fail(404, createFormError('CHALLENGE_NOT_FOUND', 'Frame not found'));
	}

	if (challenge.type !== ChallengeType.FRAMED || frameChallenge.type !== ChallengeType.FRAMED) {
	...
	return { challenge: frameChallenge, challengeHasAnotherFrame, responses };
};
